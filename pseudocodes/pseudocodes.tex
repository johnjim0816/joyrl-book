...
\begin{algorithm}[H] % [H]固定位置
	...
	\begin{algorithmic}[1] % [1]显示步数
		...
		\FOR {迭代数 = $1,2,\cdots,L$}
			\STATE 初始化状态$s_1$
			\FOR {回合数 = $1,2,\cdots,T$}
				\STATE 智能体$u$根据策略$\pi_{\theta_{old}^u}^u$执行动作$a_t^u$，获得奖励$r_{t+1}$并进入下一状态$o_{t+1}^u$
			...
			\FOR {epoch数 $ k = 1,2,\cdots,K$}
				...
				\FOR {$j=0,1,\cdots,\frac{T}{B}-1$}
					...
					\STATE 计算梯度：
					\STATE $\bigtriangleup\theta^u=\frac{1}{B}\sum_{i=1}^B\{\bigtriangledown_{\theta^u}f(r_i(\theta^u),A_i^u)\}$\footnotetext[4]\break
				...
				\ENDFOR
			\ENDFOR
		\STATE 更新$\theta_{old}^u\leftarrow\theta^u$, $\hat{\phi}^u\leftarrow\phi^u$
		\ENDFOR
	\end{algorithmic}
\end{algorithm}
